...
mod tests {
    ...
    fn test_parse_artists() {
        ...
        assert_eq!(parse_artists("Artist One feat. Artist Two").unwrap(), vec!["Artist One", "Artist Two"]);
        assert_eq!(parse_artists("Artist One & Artist Three").unwrap(), vec!["Artist One", "Artist Three"]);
    }
}